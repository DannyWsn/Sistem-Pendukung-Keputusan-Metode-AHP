...
<script>
    ...
    $("#formentri").submit(function(e) {
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: "<?= base_url(); ?>Perbandingan/updatealt",
            data: $(this).serialize(),
            error: function() {
    ...
    $("#prioform").submit(function(e) {
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: "<?= base_url(); ?>Perbandingan/updatealtprioritas",
            data: $(this).serialize(),
            error: function() {
...
<div id="entri" class="col-md-12">
            ...
            <tbody>
                ...
                foreach ($arr as $k2 => $v2) {
                    ...
                    for ($i = 1; $i <= $jumlah; $i++) {
                        ...
                        $noSub += 1;
                        if ($noSub == $noUtama) {
                            echo '<td><input type="number" min=1 id="k' . $noUtama . 'b' . $noSub . '" class="bg-blue form-control form-control-sm kolom' . $noSub . '" value="1" readonly="" title="kolom' . $noSub . '"/></td>';
                        } else {

                            $nilai = ambil_nilai_alternatif($subkriteriaid, $k2, $xxx);
                            echo '<td><input type="text" name="' . $newname . '" id="k' . $noUtama . 'b' . $noSub . '" class="bg-white form-control form-control-sm inputnumber kolom' . $noSub . '" data-target="k' . $noSub . 'b' . $noUtama . '" data-kolom="' . $noSub . '" value="' . $nilai . '" title="kolom' . $noSub . '"/></td>';
                        }
                    }
                    echo '</tr>';
                }
    ...
    <div class="pull-left">
        ...
        <a href="javascript:;" onclick="showmatrik();" class="btn btn-info">Lihat Matriks</a>
